...
Formula read_cnf(const Header &header, std::ifstream &input_file) {
    Formula formula;
    Clause clause;

    int number_of_clauses = 0;
    process_lines(input_file, [&header, &formula, &clause, &number_of_clauses](std::istringstream &iss) {
        int variable;
        while (!iss.eof() && iss >> variable) {
            if (variable == 0) {
                ...
                clause.add_literal(Literal(absolute_variable, absolute_variable != variable));
            }
        }
        if (iss.bad()) {
            std::string rest;
...
Formula DimacsParser::parse_dimacs_file(std::ifstream &input_file) {
